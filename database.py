...
def user_playlists(username):
    ...
    try:
        ...
        ###############################################################################
        sql = """
        select collection_id, collection_name, count(media_id) from mediaserver.MediaCollection natural join mediaserver.MediaCollectionContents where username= %s group by collection_name,collection_id
        """
...
def get_alltvshows():
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT tvshot_id, tvshow_title, count(tvshow_title) AS "episode count"
...
def get_song(song_id):
    ...
    try:
        ...
        #############################################################################
        sql = """select s.song_id, s.song_title as "Song_Title", s.length as "Song_Length", a.artist_name as "Artist",
        string_agg(mt.md_value, ',' ORDER BY mt.md_id) AS "Genres"
...
def get_song_metadata(song_id):
    ...
    try:
        ...
        #Need to add album artwork retrieval to query

        sql = """
        select s.song_id, s.song_title as "Song_Title", s.length as "Song_Length", a.artist_name as "Artist",
        string_agg(mt.md_value, ',' ORDER BY mt.md_id) AS "Genres"
        from mediaserver.song s
            inner join mediaserver.song_artists sa on (sa.song_id = s.song_id)
            inner join mediaserver.MediaItemMetaData m on (m.media_id = s.song_id)
...
def get_podcast(podcast_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT P.podcast_id, P.podcast_title, P.podcast_uri, P.podcast_last_updated, T.md_type_name, D.md_value
...
def get_album_songs(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """select A.album_id, A.album_title, string_agg(concat(asg.track_num , s.song_title), ',' ORDER BY asg.track_num)
            from album A 
	        inner join Album_Songs asg on (A.album_id = ASg.album_id) 
	        inner join Song s on (asg.song_id = s.song_id)
            group by a.album_id
            order by A.album_id;
        """
...
def get_tvshow(tvshow_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT tvshow_title, string_agg(md_type_name || ': ' || md_value, ', ') AS "Show Metadata"
        FROM mediaserver.TVShow NATURAL JOIN mediaserver.TVShowMetaData NATURAL JOIN mediaserver.MetaDataType NATURAL JOIN mediaserver.MetaData
        WHERE tvshow_id = %s
...
#   Add a new Song
#####################################################
def add_song_to_db(location,songdescription,title,songlength,songgenre, artistid,artwork,artistname):
    """
    Add a new Movie to your media server
    """
    conn = database_connect()
    ...
    try:
        ...
        SELECT
            mediaserver.addSong(
                %s,%s,%s,%s,%s,%s,%s,%s);
        """
        cur.execute(sql,(location,songdescription,title,songlength,songgenre, artistid,artwork,artistname))
        conn.commit()                   # Commit the transaction
        r = cur.fetchone()
        print("return val is:")
        print(r)
